    ...
    var maku_list=["導演","場務","場控","舞監","服化","攝影","特技"];
      ...
      switch(n){
        ...
        case 2:
          ...
          if(document.getElementById("btn1") == null)
          {
            ...
            btn.setAttribute("onclick","btn_click("+1+")");
    ...
    function situ_EX()
    {
      document.body.removeChild(document.getElementById("btn" + counter_btn))
      
      set_inner("scene","請選擇你的貢丸人類型");
      set_posi("scene","10 * var(--vh)","calc((50 * var(--vw)) - (8 * var(--vh)))");
      var btn;
      for(var i=0;i<7;i++){
        btn=document.createElement("button");
        btn.setAttribute("type","button");
        btn.setAttribute("id","btn"+(i+1));
        btn.setAttribute("onclick","result_str=maku_list["+i+"];"+"QRinfo[16]="+i+"; step=9; del_ex_btn(); btn_click(2);");
        // result_str="+maku_list[i]
        
        document.body.appendChild(btn);
      }
      for(var i=0;i<7;i++){
        set_size("btn"+(i+1),"8 * var(--vh)","20 * var(--vh)");
        set_posi("btn"+(i+1),17+i*11 +" * var(--vh)","calc((50 * var(--vw)) - (10 * var(--vh)))");
        set_inner("btn"+(i+1),maku_list[i]);
      }
    }
    ...
    function skip_btn(){
      var btn=document.createElement("button");
      btn.setAttribute("type","button");
      btn.setAttribute("id","btn"+99);
      btn.setAttribute("onclick","del_img(); del_skip_btn(); step=20; btn_click(1);");
      // btn.setAttribute("onclick","choose(2)");
      document.body.appendChild(btn);
    ...
    function del_ex_btn(){
      for(var i=0;i<7;i++){
        document.body.removeChild(document.getElementById("btn"+(i+1)));
      }
      set_inner("scene","");
    }
    ...
    function del_btn(){
      if(document.getElementById("btn99"))
      {del_skip_btn()}
      counter_btn--;
      var btn=document.getElementsByTagName("button");
      if(btn[counter_btn] != null)
      {
